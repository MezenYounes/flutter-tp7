    ...
    body: jsonEncode(<String, dynamic>{
      ...
      "matieres": List<dynamic>.from(
        matieres.map((matiere) => {
              "codMat": matiere
                  .codMat, // Assurez-vous de passer les identifiants des matières
            }),
      ),
    }),
  );
...
Future updateClasse(Classe classe, List<Matiere> matieres) async {
  ...
  if (response.statusCode == 200) {
    ...
    await http.delete(Uri.parse(
        "http://localhost:8081/classMat/delete?codClass=${classe.codClass}"));
...
Future<List<Matiere>> getMatiereByClasse(int codClass) async {
  final response = await http
      .get(Uri.parse('http://localhost:8081/classes/$codClass/matieres'));
  if (response.statusCode == 200) {
