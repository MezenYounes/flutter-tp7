...
Future getAllStudent() async {
  Response response =
      await http.get(Uri.parse("http://localhost:8081/etudiant/all"));
  return jsonDecode(
      response.body); // Accédez à la clé contenant la liste d'étudiants
}
...
Future addStudent(Student student) async {
  print(
      "Nom: ${student.nom}, Prénom: ${student.prenom}, Date de naissance: ${student.dateNais}");
  print(
      "ID Classe: ${student.classe?.codClass}"); // Vérifiez que la classe est bien sélectionnée
  ...
  print(
      "Réponse du serveur : ${response.body}"); // Affiche la réponse pour débogage
  ...
  final response = await http.put(
    ...
    body: jsonEncode({
      ...
      "classe": student.classe != null
          ? {
              "codClass": student
                  .classe!.codClass, // Ensure the 'classe' is mapped correctly
            }
          : null,
    }),
  );
