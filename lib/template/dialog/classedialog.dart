...
class ClassDialog extends StatefulWidget {
  ...
  ClassDialog(
      {required this.notifyParent, this.classe, required this.matieres});
...
class _ClassDialogState extends State<ClassDialog> {
  ...
  void initState() {
    ...
    if (widget.classe != null) {
      ...
      _selectedMatieres = widget.matieres
          .where((matiere) =>
              widget.classe!.matieres != null &&
              widget.classe!.matieres!.contains(matiere.codMat))
          .toList();
    }
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(
          widget.classe == null ? 'Ajouter une classe' : 'Modifier la classe'),
      content: Form(
        key: _formKey,
        child: Column(
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return MultiSelectDialogField(
                    items: snapshot.data!.map((Matiere matiere) {
                      return MultiSelectItem<Matiere>(matiere,
                          matiere.intMat); // Afficher le nom de la matière
                    }).toList(),
          ...
          onPressed: () async {
            if (_formKey.currentState!.validate()) {
              if (widget.classe == null) {
                ...
                final classe = Classe(
                  _nbreEtudController.text.isNotEmpty
